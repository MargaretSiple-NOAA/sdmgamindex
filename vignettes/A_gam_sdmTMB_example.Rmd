...
#dat2 <- readRDS(here::here("data/wcvi-dogfish.rds"))
#head(dat2)

# If the ebs_yfs_example.rds file isn't available:
#load(here::here("data/noaa_afsc_public_foss.rda"))
#yfs <- noaa_afsc_public_foss |> dplyr::filter(srvy=="EBS" & species_code == 10210 )
...
start.time <- Sys.time()
fit_gam <- gam(
  formula = cpue_kgha ~ as.factor(year) + # for fitting biomass, use gamma distribution and log link
    s(X, Y), #+ offset(log(area_swept_ha)), 
...
fit_gam_st <- gam(
  formula = count ~ s(depth_m) + as.factor(year) +
    s(X,Y, by = year), 
  family = tw(link = "log"),
 # offset = "area_swept_ha", #another way to add offset? I guess?
  data = dat)
...
sum(pred_gam_df$predicted_tot_grid)/1000
...
ggplot(aes(X, Y, color=pred_gam)) + 
  ...
  #facet_wrap(~year) + 
  coord_fixed() +
  labs(color = "Log Biomass density\n(kg/km^2)") +
  theme_light() +
...
## Get uncertainties by sampling from posteriors

[`{gratia}`](https://gavinsimpson.github.io/gratia/) is an R package for evaluating and displaying GAM fits. This script uses the `fitted_samples()` function, which draws fitted values from the posterior of a model using a Gaussian approximation. 
