...
```{r}
grid <- purrr::map_dfr(unique(dat$year), ~ tibble(pred_grid_ebs, year = .x))
```
...
```{r}
grid2 <- grid
grid2 <- grid |> rename(longitude_dd = "lon", latitude_dd = "lat")
pred_gam <- predict(fit_gam, type = "response", newdata = grid2)
pred_gam_df <- cbind(grid2, pred_gam)
```
...
pred_gam_df %>%
  filter(year==1999) |>
ggplot(aes(longitude_dd, latitude_dd, fill = pred_gam)) + 
  geom_tile() + # Sean uses geom_tile or geom_stars if it's a stars object
