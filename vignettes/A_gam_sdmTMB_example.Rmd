...
> Adapted from [GitHub pbs-assess/sdmTMB-teaching 2023-02-14](https://github.com/pbs-assess/sdmTMB-teaching/blob/490ee04157f19d1059c55a1318b894e8b8f460bb/uw-survey-2023/02-exercise.qmd#L356)
...
```{r}
ggplot(data = dat, 
       mapping = aes(x = longitude, y = latitude, size = density)) + 
  geom_point()
```
...
# Predict to full survey area (new data)

Load a prediction grid for the full survey area:
```{r}
wcvi_grid <- readRDS(here::here("notforgit/uw-survey-2023/data/wcvi-grid.rds"))
head(wcvi_grid)
wcvi_grid$log_depth <- log(wcvi_grid$depth)

# Add year to the grid using purrr (yay)
...
```{r}
sdmTMB::add_utm_columns(dat, ll_names = c("longitude", "latitude"))
```
...
# ggplot alternative:
ggplot() + 
  inlabru::gg(mesh$mesh) + coord_fixed() +
