...
```{r model_2_fit_load, echo = FALSE}
load(system.file(paste0("vigB_model_fits_fm_2.Rdata"), 
                package = "sdmgamindex"), verbose = TRUE)
```

```{r model_2_fit_use, eval = TRUE}
cat("The GAM took ", fittime, "mins to run")
...
```{r model_3_fit_load, echo = FALSE}
load(system.file(paste0("vigB_model_fits_fm_3.Rdata"), 
                package = "sdmgamindex"), verbose = TRUE)
```

```{r model_3_fit_use, eval = TRUE}
cat("The GAM took ", fittime, "mins to run")
...
```{r model_1_pred_load, echo = FALSE}
...
```{r model_1_sim_load, echo = FALSE}
load(system.file(paste0("vigB_model_sim_fm_1.Rdata"), 
                package = "sdmgamindex"), verbose = TRUE)
```

```{r model_1_sim_use, eval = TRUE}
sims_sum
```
...
```{r model_4_mesh_plot, eval = TRUE}
mesh <- make_mesh(dat, c("X", "Y"), cutoff=10)
plot(mesh)
```

```{r model_4_fit, eval = FALSE, cache = TRUE}
start.time <- Sys.time()
...
```{r model_4_fit_load, echo = FALSE}
load(system.file(paste0("vigB_model_fits_fm_4.Rdata"), 
                package = "sdmgamindex"), verbose = TRUE)
```

```{r model_4_fit_use, eval = TRUE}
cat("The GAM took ", fittime, "mins to run")
sdmTMB::sanity(fit)
tidy(fit)
