...
:::: {style="display: flex;"}
::: {style="width:500px;"}
![Yellowfin Sole](https://github.com/EmilyMarkowitz-NOAA/gap_bs_data_report/blob/main/img/yellowfin_sole.png?raw=true){width=25%}
:::
::: {}
This commonly caught "golden child" species has been realitively easy to model and assess because of it's high abundance, large distribution over the survey area, and consistent availability to the survey. 
:::
::::
...
:::: {style="display: flex;"}
::: {style="width:500px;"}
![Walleye Pollock](https://github.com/EmilyMarkowitz-NOAA/gap_bs_data_report/blob/main/img/walleye_pollock.png?raw=true){width=25%}
:::
::: {}
...
:::: {style="display: flex;"}
::: {style="width:500px;"}
![Red King Crab](https://github.com/EmilyMarkowitz-NOAA/gap_bs_data_report/blob/main/img/red_king_crab.png?raw=true){width=25%}
:::
::: {}
This "problem child" will have patchier and less understood availability to the survey. 
:::
::::
...
For the sake of a simple example, we will only assess data from 2010 to 2021. 
...
plot(extrap_data0) # Just so we can see what we are looking at: 
...
```{r covar_bt}
x <- coldpool::ebs_bottom_temperature %>%
  raster::projectRaster(., crs = crs_latlon) %>%
  raster::extract(x = ., 
                  y = as.matrix(dat_cov[,c("sx", "sy")])) %>% 
  as.data.frame()

names(x) <- paste0("GEAR_TEMPERATURE", 
                          gsub(pattern = "_", replacement = "", 
                               x = gsub(pattern = "[a-zA-Z]+", 
                                        replacement = "", 
                                        x = names(x))))
dat_cov <- dplyr::bind_cols(dat_cov, x) %>% 
  na.omit()
...
get_pd <- function(year, 
 ...
 allpd <- lapply(YEARS, FUN = get_pd, x = dat_cov, vars = vars, varsbyyr = varsbyyr)
...
#   # Null model spatial
#   "fm_1null_s" = #"Year +
#     "s(sx,sy,bs=c('ts'),k=520)",
#   # Null model temporal
#   "fm_1null_t" = "Year",
#   # Null model spatial and temporal
#   "fm_1null_s_t" = "Year +
# s(sx,sy,bs=c('ts'),k=520)",
# 
#   # Null model spatial and temporal with an additional spatiotemporal effect
#   "fm_1null_s_t" = "Year +
# s(sx,sy,bs=c('ts'),k=520)  + 
...
s(log(GEAR_TEMPERATURE+3),bs='ts',k=10)"#,
  # Model with an additional year effect
#   "fm_3smyr" = "Year + 
# s(sx,sy,bs=c('ts'),k=520) + 
# s(BOTTOM_DEPTH,bs='ts',k=10) + 
# s(log(GEAR_TEMPERATURE+3),bs='ts',k=10) + 
# s(Year, bs=c('ts'), k=10)",
#   # model with an additional spatiotemporal effect
#   "fm_4st" = "Year + 
# s(sx,sy,bs=c('ts'),k=520) + 
# s(BOTTOM_DEPTH,bs='ts',k=10) + 
# s(log(GEAR_TEMPERATURE+3),bs='ts',k=10)  +
...
```{r model_fit}
  ...
  for(i in 1:nrow(comb)){
    ...
    fittimes[[ temp ]] <- 
      system.time ( models[[ temp ]] <- 
                     surveyIndex::get_surveyidx(x = ds[[ comb$SPECIES[i] ]], 
                                   ages = 1,
                                   myids = NULL, 
                                   predD = allpd,
                                   cutOff = 0,
  ...
  save(models, fittimes, file = paste0(dir_out, "model_fits.Rdata"))
```
