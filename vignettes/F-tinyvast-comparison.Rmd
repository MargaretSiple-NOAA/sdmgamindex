...
integrate_output(
  ...
  bias.correct = FALSE
) # bias.correct=FALSE means no random effects
...
# Pull db estimates straight from GAP_PRODUCTS
library(RODBC)
channel <- gapindex::get_connected()
dbdat <- RODBC::sqlQuery(
  channel = channel,
  query = "
...
# tinyVAST
# dev.off()
plot(YEARS, Est[1, ], type = "b", ylab = "Estimated biomass (mt)", pch = 19, ylim = c(1e7, 3e8))
arrows(
  x0 = YEARS, x1 = YEARS, y0 = Est[1, ] - Est[2, ],
...
arrows(
  x0 = YEARS + 0.1, x1 = YEARS + 0.1, y0 = dbdat$BIOMASS_CI_DW * 1e8,
...
`dat` is our sample dataset created above in the "GAM" section. This data is formatted and processed analogously to the data in the "Intro to GAMs" vignette.
...
ll <- sdmgamindex::convert_crs( 
  x = dat0$longitude_dd_start,
  y = dat0$latitude_dd_start, 
  crs_in = crs_latlon, 
  crs_out = crs_proj) 

YEARS <- sort(unique(dat0$year))
...
dat_wrangled <- dat0 |> 
  ...
  dplyr::select(wCPUE, GEAR_TEMPERATURE, BOTTOM_DEPTH, COMMON_NAME, EFFORT, 
                Year, Ship, Lon, Lat, sx, sy, 
                ctime, TimeShotHour, timeOfYear, Gear, Quarter, HaulDur, hauljoin)
head(dat_wrangled)
```

### Retrieve the GOA prediction grid

```{r}
pg <- read.csv(here::here("data/pred_grid_goa.csv"),header = TRUE)
pred_grid_goa <- pg |> 
  rename(lon = ShapeLongitude, lat = ShapeLatitude) |>
  dplyr::select(lon, lat, Shape_Area)
# For the future, will add this to the package files, and I don't think we would need pred_grid_goa.csv after that.
# save(pred_grid_goa, file = here::here("data/pred_grid_goa.rda"))

load(here::here("data/pred_grid_ebs.rda"))
pred_grid_ebs


pred_grid <- sdmgamindex::pred_grid


```

### Fit the SDM using `sdmgamindex`

```{r}

dat_datras <- sdmgamindex::get_datrasraw(x = dat_wrangled)
