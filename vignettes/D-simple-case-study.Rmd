---
title: "Simple {sdmgamindex} case study"
date: "`r format(Sys.Date(), format='%B %d %Y') `"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Simple {sdmgamindex} case study}
...
save(extrap_data0, extrap_data, 
     file = here::here("inst",
                       paste0("VigA_bottom_depth_raster_", min(YEARS),"-",max(YEARS), ".rdata")))
```
...
```{r covar_bt_test}
plot(terra::unwrap(coldpool::ebs_bottom_temperature)) # Just so we can see what we are looking at: 
```

```{r covar_bt_test, tab.cap = "Adding bottom temperature covariate data. "}
tmp <- which(names(terra::unwrap(coldpool::ebs_bottom_temperature)) %in% YEARS)
...
load(system.file("VigA_model_fits.Rdata", package = "sdmgamindex") )
# load("../output/VigA_sdmgamindex_models_model_fits.Rdata")
```
...
save(models, fittimes, file = here::here("inst","VigA_model_fits.Rdata"))
```
